        ...
        for (int i = 0; i < times.Length; i++)
        {
            ...
            for (int j = 0; j < 7; j++)
            {
                DateTime delivery_time =
                    new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, time, 0, 0);
                //DateTime delivery_time = DateTime.Now.AddSeconds(times[i] + 5);
                delivery_time = delivery_time.AddDays(j);
                if (delivery_time > DateTime.Now)
                {
                    Schedule_NOTI(delivery_time, title, body);
                }
            }
        }
    }
